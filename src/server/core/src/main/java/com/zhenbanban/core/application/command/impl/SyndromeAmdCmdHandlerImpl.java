...
public class SyndromeAmdCmdHandlerImpl implements SyndromeAmdCmdHandler {
    ...
    @Override
    @Transactional
    public Long handleAdd(SyndromeAmdCommand command) {
        Long id = idGenerator.nextId();

        ModelMapper modelMapper = new ModelMapper();
        Syndrome syndrome = modelMapper.map(command, Syndrome.class);
        syndrome.setId(id);
        syndrome.add();

        syndromeRepository.save(syndrome, true);
    ...
    @Override
    @Transactional
    public void handleModify(SyndromeAmdCommand command) {
        Syndrome syndrome = syndromeRepository.load(command.getId());

        ModelMapper modelMapper = new ModelMapper();
        modelMapper.map(command, syndrome);
        syndrome.modify();
    ...
    public void handleDestroy(Long id) {
        Syndrome syndrome = syndromeRepository.load(id);
        syndrome.destroy();
