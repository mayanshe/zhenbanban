...
/**
 * Query Achieve : 疾病诊断查询处理器实现类
 *
 * @author zhangxihai 2025/8/15
 */
@Service
@RequiredArgsConstructor
public class DiagnoseQueryHandlerImpl implements DiagnoseQueryHandler {
    private final DiagnosePoMapper mapper;

    @Override
    public DiagnoseDto handleQuerySingle(Long id){
        DiagnosePo po = mapper.findById(id);
        if (po == null) {
            throw new BadRequestException("没有找到此疾病诊断");
        }

        return (new ModelMapper()).map(po, DiagnoseDto.class);
    }

    @Override
    public List<DiagnoseDto> handleQueryList(DiagnoseQuery query) {
        throw new UnsupportedOperationException("handleQueryList method is not implemented yet");
    }

    @Override
    public Pagination<DiagnoseDto> handleQueryPage(DiagnoseQuery query) {
