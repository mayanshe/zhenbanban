...
public class MedicineQueryHandlerImpl implements MedicineQueryHandler {
    private final MedicinePoMapper mapper;

    @Override
    public MedicineDto handleQuerySingle(Long id){
        MedicinePo po = mapper.findById(id);
        if (po == null) {
            throw new BadRequestException("没有找到此药品信息");
        }

        return (new ModelMapper()).map(po, MedicineDto.class);
    }

    @Override
    public List<MedicineDto> handleQueryList(MedicineQuery query) {
        throw new UnsupportedOperationException("handleQueryList method is not implemented yet");
    }

    @Override
    public Pagination<MedicineDto> handleQueryPage(MedicineQuery query) {
        return Pager.paginate(mapper, query.getPage(), query.getPageSize(), query.toMap(),
                source -> (new ModelMapper()).map(source, MedicineDto.class));
    }
