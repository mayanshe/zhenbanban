<template>
  <div class="container">
    <Breadcrumb :items="['互联网医院', '互联网医院配置']" page-name="互医配置" route-name="InternetHospitalSetting"/>
    <a-card class="general-card" title="互联网医院配置">
      <a-form layout="horizontal" :model="formData" :rules="rules" ref="formRef">
        <a-row :gutter="72">
          ...
          <a-col :span="12" style="border-left: dotted 1px #ccc">当前仅提供必要的互联网医院配置，其他配置请自行添加。
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="24" style="text-align: center; margin: 24px 0">
            <a-button @click="handleSubmit" v-if="buttons.includes('internet-hospital-setting:modify')"
                      status="success">修改互联网医院配置
            </a-button>
          </a-col>
        </a-row>
      </a-form>
