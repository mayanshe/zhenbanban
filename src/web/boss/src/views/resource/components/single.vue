    ...
    <div>
      ...
      <a-form layout="vertical" :model="formData" :rules="rules" ref="formRef">
        <a-form-item field="parentId" label="父资源 : ">
          <a-cascader v-model="formData.parentId" :options="parents" placeholder="请选择" />
        </a-form-item>
        ...
        <a-form-item field="path" label="路径 : "
                     v-if="formData.resourceType === 'menu' || formData.resourceType === 'component'">
        ...
        <a-form-item field="showInMenu" label="是否显示在菜单中 : "
...
const load = () => {
  if (props.action === 'add') {
    ...
    getResource(props.singleId).then((res) => {
      formData.value = res
    }).catch((err) => {
      formData.value = generate()
      handleClose()
